...
class Welow_SuperExcel {
    ...
    private static function escribir_instrucciones( Welow_Xlsx_Writer $w ) {
        ...
        $lineas = array(
            ...
            '  CREAR (modelo nuevo)              ACTUALIZAR (modelo existente)',
            '  ─────────────────────             ──────────────────────────────',
            '  • slug vacío → usa el nombre      • Cualquier campo vacío → SE PRESERVA',
            '  • texto_enlace vacío → "Ver modelo"  (no borra los datos antiguos).',
            '  • orden vacío → 0                 • Esto permite exportar todos los modelos,',
            '  • activo vacío → 1 (activo)         editar solo 2-3 campos, y reimportar',
            '  • resto → quedan vacíos             sin perder el resto de datos.',
            '',
            '  Excepción: para BORRAR un campo en update, pon un guion: -',
            '  (todavía no implementado; por ahora edita el modelo desde admin)',
            '',
