 ...
 * CHANGELOG
 * ---------
 * 2.54.1 — Badge reCAPTCHA: solo cuadrado (oculta panel desplegable)
 *   CSS .grecaptcha-badge: width:70px + overflow:hidden + hover:70px.
 *   Mantiene el icono visible (cumple Google ToS) y elimina el panel
 *   "Protected by reCAPTCHA — Privacy — Terms".
 *
 * 2.54.0 — Footer: selector de página + URL manual para enlaces legales
 *   Configuraciones → Footer → Política / Aviso legal / Cookies:
 *   ahora muestra un desplegable con todas las páginas publicadas. Al
 *   elegir una, su permalink se vuelca al input de URL. Sigue siendo
 *   posible escribir una URL externa manualmente.
 *
 * 2.53.1 — Badge reCAPTCHA reposicionado a la izquierda (bottom-left)
 *   Google v3 lo coloca por defecto en bottom-right y no expone un parámetro
 *   URL para cambiarlo. Override CSS en formulario.css:
 *     .grecaptcha-badge { left: 14px; right: auto; bottom: 14px; }
 *
 ...
 *   - Distancia simétrica: gap 28px a un lado de la línea vertical y
 *     padding-left 28px al otro lado (antes asimétrico por justify-end).
 *   - Logos de marca ahora justify-start (empiezan justo después de la línea)
 *     para aprovechar el espacio.
 *   - Pin SVG: 18→22px, stroke 1.6→1.5. Sigue siendo vector inline (no rasterizado).
 *
 * 2.48.0 — Footer: 4 ajustes finos
 *   - Título de ubicaciones: si lo dejas vacío, no se renderiza nada (antes
 *     caía a "Nuestras ubicaciones" por defecto).
 *   - Icono pin: emoji 📍 sustituido por SVG outline (estilo línea) más limpio
 *     y consistente con el resto del diseño.
 *   - Más cuerpo al texto: títulos de columna 15→17px, nombre de ubicación
 *     14→16px, dirección/tel 13→14px, enlaces de menú 14→16px.
 *   - Reducida la distancia entre logo de empresa y logos de marcas
 *     (gap 32→18px, padding-left 32→18px).
 *
...
define( 'WELOW_CONC_VERSION', '2.54.1' );
